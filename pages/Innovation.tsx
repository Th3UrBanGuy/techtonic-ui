...
const Innovation = () => {
  const techStack = [
    { name: 'MERN STACK', ver: 'V_8.4 // SCALING', icon: Layers, color: 'text-cyan-600 dark:text-cyan-400', border: 'group-hover:border-cyan-500/50' },
    { name: 'NEXT.JS', ver: 'V_14 // OPTIMIZED', icon: Zap, color: 'text-slate-700 dark:text-white', border: 'group-hover:border-white/50' },
    { name: 'CLOUD NATIVE', ver: 'TERRAFORM // DEPLOYED', icon: Cloud, color: 'text-blue-600 dark:text-blue-400', border: 'group-hover:border-blue-500/50' },
    { name: 'RUST CORE', ver: 'LOW_LATENCY // STABLE', icon: Cpu, color: 'text-orange-600 dark:text-orange-400', border: 'group-hover:border-orange-500/50' },
    { name: 'GRAPH DB', ver: 'NEO4J // CONNECTED', icon: Network, color: 'text-purple-600 dark:text-purple-400', border: 'group-hover:border-purple-500/50' },
    { name: 'ROBOTICS OS', ver: 'ROS_2 // ACTIVE', icon: Bot, color: 'text-green-600 dark:text-green-400', border: 'group-hover:border-green-500/50' },
  ];

  const roadmap = [
    { 
      id: 'R-2025-A', q: 'Q1 2025', title: 'QUANTUM ENCRYPTION', 
      desc: 'Post-quantum cryptographic layers for the core cyber infrastructure.',
      progress: 65, status: 'IN_PROGRESS',
      color: 'from-blue-100 to-blue-50 dark:from-blue-600/20 dark:to-blue-900/10'
    },
    { 
      id: 'B-2026-X', q: 'Q4 2026', title: 'BIOTECH INTERFACES', 
      desc: 'Neural link protocols for seamless man-machine integration in robotic wings.',
      progress: 15, status: 'INITIAL_CONCEPT',
      color: 'from-purple-100 to-purple-50 dark:from-purple-600/20 dark:to-purple-900/10'
    },
    { 
      id: 'S-2027-V', q: '2027 PROJECT', title: 'AUTONOMOUS COLONIES', 
      desc: 'Self-repairing software architecture with genetic algorithmic evolution.',
      progress: 0, status: 'SCHEDULED',
      color: 'from-slate-200 to-slate-100 dark:from-gray-700/20 dark:to-gray-900/10'
    },
    { 
      id: 'G-2028-Z', q: '2028 VIS', title: 'GENETIC ALGO', 
      desc: 'Synthesis of biological data storage with silicon based processors.',
      progress: 0, status: 'HYPOTHETICAL',
      color: 'from-slate-200 to-slate-100 dark:from-gray-700/20 dark:to-gray-900/10'
    },
  ];
                  ...
                  <div className="relative h-full p-5 flex flex-col justify-between z-10">
                     ...
                     <div>
                        ...
                        <div className="flex items-center justify-between">
                           <span className="text-[9px] font-mono text-slate-500 dark:text-gray-500 bg-slate-200 dark:bg-black/40 px-2 py-1 rounded border border-slate-300 dark:border-gray-800/50">{tech.ver.split(' // ')[0]}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {roadmap.map((item, i) => (
              <motion.div 
                ...
                transition={{ delay: i * 0.1 }}
                className={`relative h-[420px] rounded-2xl p-6 border border-slate-200 dark:border-gray-800 bg-gradient-to-b ${item.color} overflow-hidden group hover:border-slate-400 dark:hover:border-gray-600 transition-colors shadow-sm dark:shadow-none`}
              >
                <div className="flex justify-between items-start mb-12">
                   <span className="text-[9px] font-mono text-slate-500 dark:text-gray-500 uppercase">REF: {item.id}</span>
                   {item.progress > 0 && (
                ...
                <div className="mb-4">
                   <span className="px-2 py-1 bg-blue-600 text-white text-[10px] font-mono font-bold rounded-sm uppercase tracking-wider">{item.q}</span>
                </div>
